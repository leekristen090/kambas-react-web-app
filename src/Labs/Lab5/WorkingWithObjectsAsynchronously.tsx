    ...
    return (
        <div id={"wd-asynchronous-objects"}>
            ...
            <div className={"form-check form-switch"}>
                <input className={"form-check-input"} type={"checkbox"} id={"wd-complete"}
                       defaultChecked={assignment.completed}
                       onChange={(e) =>
                           setAssignment({...assignment, completed: e.target.checked})} />
                <label htmlFor={"wd-complete"} className={"form-check-label"}>Completed</label>
            </div>
            <button className={"btn btn-primary me-2"}
                    onClick={() => updateTitle(assignment.title)}>
                Update Title
            </button>
            <pre>{JSON.stringify(assignment, null, 2)}</pre><hr />
        </div>
    );
}